...
function prettyError(errorInstance) {
    if (errorInstance instanceof globals_1.Error && errorInstance.stack !== undefined) {
        return errorInstance.stack;
    }
    try {
        return (0, globals_1.String)(errorInstance);
    }
    catch (_err) {
    }
    if (errorInstance instanceof globals_1.Error) {
        try {
            return (0, globals_1.safeErrorToString)(errorInstance);
        }
        catch (_err) {
        }
    }
    if (errorInstance !== null && typeof errorInstance === 'object') {
        try {
            return (0, globals_1.safeToString)(errorInstance);
        }
        catch (_err) {
        }
    }
...
function preFormatFailure(out, stringifyOne) {
    ...
    const messageErrorPart = includeErrorInReport
        ? `\nGot ${(0, globals_1.safeReplace)(prettyError(out.errorInstance), /^Error: /, 'error: ')}`
        : '';
