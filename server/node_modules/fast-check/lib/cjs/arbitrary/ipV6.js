...
function h16sTol32Unmapper(value) {
    ...
    if (!value.includes(':'))
        throw new globals_1.Error('Invalid value');
...
function hexa() {
    if (cachedHexa === undefined) {
        cachedHexa = (0, integer_1.integer)({ min: 0, max: 15 }).map((n) => items[n], (c) => {
            if (typeof c !== 'string') {
                throw new globals_1.Error('Not a string');
            }
            if (c.length !== 1) {
                throw new globals_1.Error('Invalid length');
            }
            const code = (0, globals_1.safeCharCodeAt)(c, 0);
            if (code <= 57) {
                return code - 48;
            }
            if (code < 97) {
                throw new globals_1.Error('Invalid character');
            }
            return code - 87;
        });
    }
...
/**@__NO_SIDE_EFFECTS__*/function ipV6() {
    const h16Arb = (0, string_1.string)({ unit: hexa(), minLength: 1, maxLength: 4, size: 'max' });
